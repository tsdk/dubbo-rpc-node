...
class ZookeeperRegistry extends Base {
  ...
  async _init() {
    const tid = setTimeout(() => {
     throw new Error(`zookeeper [${this.options.address}] address not available`);
    }, 5000);
  ...
  subscribe(config, listener) {
    ...
    if (!this._subscribeMap.has(interfaceName)) {
      ...
      const consumerUrl = fmt('%s://%s?uniqueId=%s&version=%s&pid=%s&timeout=%s&appName=%s&startTime=%s',
  ...
  _buildProviderPath(config) {
    return this._rootPath + encodeURIComponent(config.interfaceName) + '/providers';
  }

  _buildConsumerPath(config) {
    return this._rootPath + encodeURIComponent(config.interfaceName) + '/consumers';
  }
  ...
  _isMatch(consumer, urlStr) {
    ...
    if (interfaceName && consumer.interfaceName !== interfaceName) {
      if (encodeURIComponent(interfaceName) !== consumer.interfaceName)
        return false;
    }
