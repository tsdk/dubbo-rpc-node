...
class RpcServer extends Base {
  ...
  publish() {
    ...
    for (const service of this._services.values()) {
      ...
      tasks.push(service.publish(this._getServiceUrl(service)));
    }
  ...
  _getServiceUrl(service) {
    ...
    const params = {
      ...
      anyhost: 'true',
      charset: 'UTF-8',
      loadbalance: service.options.loadbalance || 'random'
    }
    const methods = Object.keys(service.delegate).join(',');
    url = `${url}&${qs.stringify(params)}&methods=${methods}`;
    return url;
  }
}
